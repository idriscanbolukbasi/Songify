...
public class LinkedList<T extends Node<T>> {
    ...
    public LinkedList<T> add(LinkedList<T> list, T node) {
        if (list.head == null) list.head = node;
        else {
            T last = list.head;
            while (last.next != null) last = last.next; // T = O(N), since we're working with linked list we have to traverse to tail of the linked list to add
    ...
    public boolean contains(LinkedList<T> list, T node) { // T = O(N),
        T currentNode = list.head;
    ...
    public T get(LinkedList<T> list, T node) { // T = O(N),
        T currentNode = list.head;
    ...
    public T get(LinkedList<T> list, int index) { // T = O(N),
        T currentNode = list.head;
    ...
    public int size(LinkedList<T> list) { // T = O(N),
        int size = 0;
